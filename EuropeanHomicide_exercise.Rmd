...
-     As seen in the graphs below, the reigns of kings generally get longer as we get closer to the present day. At the same time, the number of murders in Europe decreases 

```{r eval = TRUE}

kings <- read_csv2("data/danske__monarker-kopi.csv")
names(kings)

kings %>% 
  mutate(duration = afsaettelse - Indsaettelse) %>% 
  mutate(midyear = afsaettelse - duration/2) %>% 
